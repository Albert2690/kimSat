...
const DepartmentDetialed3 = ({cards}) => {
  // const cards = [
  //   {
  //     icon: 'fas fa-eye',
  //     title: 'Clinical Ophthalmology Services',
  //     description: 'Our dedicated trauma bay provides urgent care for critically injured patients. Advanced imaging facilities, including CT and MRI scanners, enable rapid diagnosis and treatment.'
  //   },
  //   {
  //     icon: 'fas fa-eye',
        ...
        {visibleCards.map((card, index) => (
          <div key={index} className="bg-white transform hover:bg-thirdColor group hover:scale-110 duration-1000 p-6 rounded-lg shadow-md">
            <div className="text-center mb-4">
              <i className={`fas fa-eye text-4xl group-hover:text-white text-secondaryColor`}></i>
            </div>
            <h2 className="text-xl font-semibold text-headingColor group-hover:text-white mb-2">{card.heading}</h2>
            <p className="text-gray-600 text-justify  text-sm group-hover:text-white">{card.content}</p>
          </div>
        ))}
      </div>
