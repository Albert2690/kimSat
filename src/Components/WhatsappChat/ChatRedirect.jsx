import React,{useState} from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCommentDots } from "@fortawesome/free-solid-svg-icons"; // Available in free package

import chatDoctor from '../../assets/Png Icons/ask.png'
import { motion } from 'framer-motion';

function ChatRedirect() {
  const [isHovered, setIsHovered] = useState(false);

    const phoneNumber = "+917510992035"; 
    
    const handleClick = () => {
      const message = `Hello, I would like to Know more about the kimsat hospital. Could you please provide more detials?`;
      
      const encodedMessage = encodeURIComponent(message);
      const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
      
      window.open(whatsappUrl, '_blank');
    }
  return (
    <div className='flex flex-col gap-5 fixed bottom-7 right-4'>
    <motion.div
  
      className="relative flex items-center"
   
      onHoverStart={() => setIsHovered(true)}  // Set hover state to true
      onHoverEnd={() => setIsHovered(false)}   // Set hover state to false
    >
      {/* The Button */}
      <motion.button
      onClick={handleClick}
        className="bg-thirdColor hover:bg-secondaryColor text-white p-3 rounded-full flex space-x-2 items-center group transition-all duration-300 ease-in-out shadow-lg"
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
      >
        <FontAwesomeIcon
          icon={faCommentDots}
          className="p-1 text-sm text-white rounded-full transform transition-transform duration-500 group-hover:translate-y--2"
        />

        {/* <img src={chatDoctor} className="p-1 w-[25px] h-auto transform transition-transform duration-500 group-hover:translate-y--2 " /> */}
      </motion.button>

      {/* Detail Box - Appears on Hover */}
      {isHovered && (
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: 20 }}
          className="absolute right-12 bottom-0 bg-secondaryColor text-white p-3 rounded-lg shadow-lg text-sm w-max"
        >
          Chat with an Expert
        </motion.div>
      )}
    </motion.div>
  </div>
    
   
     
    
    
    
  
    
  )
}

export default ChatRedirect